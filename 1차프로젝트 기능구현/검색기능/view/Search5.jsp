...
<body>
	...
	<table border="1">
		...
		<tr>
			<td><button type = "submit" name = "reset" value = "reset" onclick = "reset(event)">초기화</button></td> <!-- 선택한 값 삭제   -->
			...
			$.ajax({
				...
				success : function(result){
					...
					for (let i = 0 ; i < result.length; i ++) {
						...
						if(data != null){
							$("#flavor").append("<tr>"
									+"<td><input type ='checkbox' name = 'flavor' onclick = 'checkFlavor(event)' value = '"+data+"'>"+data+"</td>"
									+"</tr>");
...
//			$("#selectTaste td").eq(1).remove();
			$("#selectTaste td").html() == tasteValue.remove();
		}
	} 
...
//선택한 향 출력하기
 	...
 	function checkFlavor(event){
		...
		if(event.target.checked == true){
			...
			$("#selectFlavor").append("<td>" + flavorValue + "</td")
			flavorList.push(flavorValue);
			console.log(flavorList);
		
		}else if(event.target.checked == false){
			flavorValue = event.target.value;
			$("#selectFlavor * " ).remove("<td>" + flavorValue + "</td");
		}
	} 
	...
	function Send(event){
		...
		if(event.target.click){
			...
			$.ajax({
				url : "ResultCon",
				traditional : true,
				data : {"num" : sort, "tasteList" : tasteList, "flavorList" : flavorList, "option" : option},
				dataType : "json",
				success : function(result){
					console.log(tasteList)
					alert("성공");
